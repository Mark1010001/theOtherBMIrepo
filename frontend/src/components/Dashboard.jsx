...
const Dashboard = ({ data, userResults, userMetrics, onLogout, isLightMode, toggleTheme }) => {
    ...
    <div className="flex-1 p-8 flex flex-col gap-10 overflow-y-auto custom-scrollbar bg-bg-main transition-colors duration-300">
      ...
      <div className="p-5 rounded-xl bg-brand/5 border border-brand/15 flex items-start gap-4">
        ...
        <p className="text-[13px] leading-relaxed text-text-muted">
          ...
          <b className="text-text-header mx-1">{patterns.disparity_count} out of {patterns.total_users} users</b>
          are classified as <b className="text-text-header">'Normal'</b> under WHO standard but
          ...
          {tabs.map(tab => (
            <button
              ...
              className={`pb-4 text-[11px] font-bold tracking-widest transition-all relative ${
                activeTab === tab ? 'text-text-header' : 'text-text-dim hover:text-text-muted'
              }`}
            >
              {tab}
              {activeTab === tab && (
